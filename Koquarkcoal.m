function  varargout = Koquarkcoal(varargin)
...
N = Kofunstat(control,param,particle,qmass);
YieldFit.D = N(TM)*2;
...
Nomega782 = Kofunstat(control,param,particle,qmass);
...
Nrho770 = Kofunstat(control,param,particle,qmass)*3;
disp(['The number of  rho0-770' ' is ' num2str(Nrho770(TM))])
YieldFit.rho770 = Nrho770(TM);

p.Ks0892.quancoal = [0 ws] ; % L and w
p.Ksp892.quancoal = [0 ws] ; % L and w
NKs = Kofunstat(control,param,p.Ks0892,qmass)+Kofunstat(control,param,p.Ksp892,qmass);
disp(['The number of  K-star-892' ' is ' num2str(NKs(TM))])

particle=p.Lam; particle.quancoal = [0 ws] ; % L and w
Nlam = Kofunstat(control,param,particle,qmass);
Nsig1385 = 6*Nlam;
Nsig1192 = 3*Nlam*1/3;
...
NLam1520 = Kofunstat(control,param,p.Lam1520,qmass);
...
NLamC = Kofunstat(control,param,p.LamC,qmass)+Kofunstat(control,param,p.SigC02455,qmass)*3+Kofunstat(control,param,p.SigC02520,qmass)*3+0.67*Kofunstat(control,param,p.LamC2625,qmass);
...
NLamB = Kofunstat(control,param,p.LamB,qmass)+Kofunstat(control,param,p.LamB5810,qmass)*3+Kofunstat(control,param,p.LamB5830,qmass)*6;  % 应该是Sigma-B 的衰变
...
N= Kofunstat(control,param,p.f0980,qmass);
...
N= Kofunstat(control,param,p.a0980,qmass)*3;   % isospin
...
N= Kofunstat(control,param,p.K01460,qmass)*2; %isospin
disp(['The number of '  p.K01460.ID ' is ' num2str(N(TM))])
Yield236(3) = NaN;
Yield458(3) = N(TM);

p.Ds2317.quancoal = [1 wc] ; % L and w
N= Kofunstat(control,param,p.Ds2317,qmass);
...
N= Kofunstat(control,param,p.Tcc3797,qmass);
disp(['The number of '  p.Tcc3797.ID ' is ' num2str(N(TM))])
Yield236(5) = NaN;
Yield458(5) = N(TM);

p.X3872.quancoal = [2 wc] ; % L and w
N= Kofunstat(control,param,p.X3872,qmass);
...
N= Kofunstat(control,param,p.Lam1405,qmass);
